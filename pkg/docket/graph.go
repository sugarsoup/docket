...
// WithObserver sets an observer to receive execution events.
// Use this to integrate with your observability stack (metrics, logs, traces).
//
// Example:
//
//	graph := docket.NewGraph(docket.WithObserver(myPrometheusObserver))
func WithObserver(observer Observer) GraphOption {
	return graphOptionFunc(func(g *Graph) {
		g.observer = observer
	})
}
...
type Graph struct {
	...
	// observer receives execution events for metrics/logging/tracing
	observer Observer
}
